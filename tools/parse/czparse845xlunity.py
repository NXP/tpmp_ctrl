...
for filename in files:
    ...
    for i in range(len(rails)):
        ...
        outputList.append("\t")
        outputList.append(str(abs(round(float(voltMax[i])/1000,3))))
        outputList.append("\t")
        outputList.append(str(abs(round(float(volt[i])/1000,3))))
        outputList.append("\t")
        outputList.append(str(abs(round(float(curMax[i])/1000,3))))
        outputList.append("\t")
        outputList.append(str(abs(round(float(cur[i])/1000,3))))
        outputList.append("\t")
        outputList.append(str(abs(round(float(pwrMax[i])/1000,3))))
        outputList.append("\t")
        outputList.append(str(abs(round(float(pwr[i])/1000,3))))
        outputList.append("\n")
        #print(volt[i])
    print(outputList)
...
for line in i2file:
    ...
    if mergedline:
        ...
        file_out_merge.write(mergename + "\t" + str(round(voltMax,3))+ "\t" + str(round(volt,3))+ "\t" + str(round(curMax,3))+ "\t" + str(round(cur,3))+ "\t" + str(round(pwrMax,3))+ "\t" + str(round(pwr,3)) + "\n")
        mergedline = False
        volt = 0
        voltMax = 0
    if not findPat:
